...
7. ~~**1.2**~~ Code fertig (a–d; Passkey optional zurückgestellt; MENSCH: Handy/Desktop, Devnet), ~~**1.3**~~ Code fertig (a–f; MENSCH: Anmeldung mit einem echten Bunker, z. B. Amber), dann Rest von **1.5** in Arbeit (a Aufzeichnung, Regeln, `test:leak` ✓, b restliche Szenarien und Aussagen mit Regel)
...
| 1.5 | Leak-Tests | in Arbeit | #23 (a) | vorher: Regeln + Fakten + DM-Szenarien; a: Aufzeichnungs-Relay (`app/test/leak/aufzeichnung.ts`), 6 neue Regeln + `LEAK_REGELN`, Szenarien DM, Anhang (echter `uploadBlob`), KI-Anfrage (echter `SessionClient`); `npm run test:leak` in der CI; 3 heutige Lecks als `todo` (2.4, 3.1 ×2); b: Raum, Swap, Profil, Abdeckung, SOL-Adresse, Aussagen mit Regel |
