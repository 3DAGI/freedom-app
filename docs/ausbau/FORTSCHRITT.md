...
| 2.4 | Verschlüsselte Anhänge | teilweise | (a: PR folgt) | a: `protocol/src/datei-krypto.ts` – `verschluesseleDatei()` (AES-256-GCM, frischer Schlüssel und Nonce je Datei, `ox` = SHA-256 des Klartexts), `entschluesseleDatei()` (GCM-Tag und Hash geprüft), `istDateiSchluessel()` für fremde Nachrichten; Abnahme-Tests: Upload ≠ Klartext (Leak-Regel über das echte `buildBlob`), Entschlüsseln klappt, Manipulation fällt auf; b folgt: App verdrahten |
