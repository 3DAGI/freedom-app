...
| 1.3 | Signer-Schnittstelle | in Arbeit | [#17](https://github.com/3DAGI/freedom-app/pull/17) (a), [#18](https://github.com/3DAGI/freedom-app/pull/18) (b), #19 (c) | a: `protocol/src/signer.ts` (Signer, SolanaSigner, LocalSigner), `state.signer` bei jeder Identität; Entscheidung 24.09.: Nip46Signer neu nach NIP-46 (Karte nannte `devices.ts`, dort gibt es keine Teile); b: `protocol/src/nip46.ts` (bunker://, Kind 24133, NIP-44, prüft jede Antwort), Anmelden per Bunker in der Oberfläche erst nach dem Umstellen; c: 37 Signier-Stellen über `state.signer` (`keypair.sk` 46 → 9 Stellen), dabei `chat-zap.ts` repariert; d DMs, e rohe Schlüssel-Stellen, f Bunker-Anmeldung |
