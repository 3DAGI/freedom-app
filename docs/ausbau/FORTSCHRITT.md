...
| 3.1 | Verschlüsselte Job-Anfragen | in Arbeit | [#25](https://github.com/3DAGI/freedom-app/pull/25) (a), #26 (b) | größte offene Datenschutzlücke; a: `protocol/src/private-job.ts` – Anfrage als Kern im Umschlag (NIP-59), versiegelt vom Sitzungsschlüssel, Rechenarbeit (NIP-13) auf dem Umschlag, `pow`-Tag im Angebot (`tiers.ts`); b: Knoten abonniert Umschläge an sich (`handlePrivate`), prüft Rechenarbeit vor dem Entschlüsseln, gratis ohne Kontingent je Schlüssel (nur wenn der Provider gratis anbietet), offene Anfragen übergangsweise weiter, `PRIVATE_POW_BITS` (Standard 12) im Angebot; c App |
