...
| 2.2a | MLS: Entscheidung und Spike | wartet auf MENSCH | (PR folgt) | Spike: ts-mls 1.6.4 (2 Mitglieder, Nachricht, Entfernen – läuft; 214 KB/67 KB gzip; kein Extensions-Draft, ohne Audit) und OpenMLS-Kern aus dem MDK-Fork als WASM (läuft in Node und Chromium; 1,43 MB/456 KB gzip; ohne `'wasm-unsafe-eval'` CompileError); MDK-Kern baut für wasm32 (81 s), aber ohne Browser-Speicher (nur SQLCipher; 16 Traits, ~140 Methoden); Marmot neu gefasst (app components, `app_data_dictionary`); Empfehlung A: MDK per WASM – MENSCH: A, B oder C in `docs/MLS-ENTSCHEIDUNG.md` ankreuzen |
